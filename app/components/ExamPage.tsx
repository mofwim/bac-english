...
import { saveExamResult, getExamResults } from '@/lib/progress';
...
export default function ExamPage({
  ...
  useEffect(() => {
    ...
    sections.forEach(async (section) => {
      const saved = await getExamResults(user.id, unitId, section.id);
      if (saved.length === 0) return;

      const answersMap: Record<string, AnswerState> = {};
      const mcqMap: Record<string, string> = {};
      ...
      saved.forEach((row) => {
        ...
        // لو كان MCQ
        const question = section.questions.find(q => q.id === row.question_id);
        if (question?.type === 'mcq' && row.user_answer) {
          mcqMap[row.question_id] = row.user_answer;
        }
      });

      setAnswers(prev => ({ ...prev, ...answersMap }));
      setSelectedMCQ(prev => ({ ...prev, ...mcqMap }));
    });
  }, [user, unitId, sections]);
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          lesson: `Bac English Exam Practice ${unitNumber}`
        }),
      });
      const data = await response.json();
      const feedback = data.reply || 'تحقق من إجابتك مع النموذج أدناه.';

      updateAnswer(questionId, 'aiFeedback', feedback);

      // حفظ في Supabase
      if (user) {
        await saveExamResult(user.id, unitId, sectionId, questionId, userAnswer, feedback);
      }
    } catch {
      updateAnswer(questionId, 'aiFeedback', 'تحقق من إجابتك مع النموذج أدناه.');
    }
    updateAnswer(questionId, 'loading', false);
  };

  const handleMCQ = async (sectionId: string, questionId: string, selected: string) => {
    setSelectedMCQ(prev => ({ ...prev, [questionId]: selected }));

    // حفظ إجابة MCQ
    if (user) {
      await saveExamResult(user.id, unitId, sectionId, questionId, selected, '');
    }
  };
    ...
    <main className="min-h-screen bg-gray-950 text-white p-4" dir="rtl">
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-purple-400 text-xs font-bold tracking-widest uppercase mb-1">
          {unitNumber} — تدرّب على الامتحان
        </p>
        <h1 className="text-xl font-bold mb-2">نمط الامتحان الحقيقي</h1>
        <p className="text-gray-400 text-sm mb-4">أجب عن الأسئلة — الـ AI يصحح إجابتك فوراً</p>

        {user ? (
          <div className="bg-green-900/20 border border-green-800 rounded-xl p-3 mb-6 text-center">
            <p className="text-green-400 text-xs">✓ إجاباتك تُحفظ تلقائياً</p>
          </div>
        ) : (
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-3 mb-6 flex items-center justify-between">
            <p className="text-gray-400 text-xs">سجّل دخولك لحفظ إجاباتك</p>
              ...
              <div className="bg-gray-900 rounded-2xl p-5 mb-4 border border-gray-800">
                ...
                <div className="space-y-2">
                  {section.summary.keyPassages.map((passage, i) => (
                    <p key={i} className="text-white text-xs font-mono bg-gray-800 rounded-xl p-3 border-r-2 border-purple-500" dir="ltr">
                      "{passage}"
                    </p>
                  ))}
                </div>
            ...
            <div className="space-y-4">
              {section.questions.map((q, idx) => (
                <div key={q.id} className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
                  ...
                  {translated[q.id] && (
                    <p className="text-gray-400 text-sm italic mb-3">{q.translation}</p>
                  )}
                  {!translated[q.id] && (
                    <button
                      onClick={() => setTranslated(prev => ({ ...prev, [q.id]: true }))}
                      className="text-xs text-gray-500 border border-gray-700 rounded-lg px-3 py-1 mb-3 hover:border-gray-500 transition-colors"
                    >
                      🌐 ترجمة
                    </button>
                  )}
                    ...
                    <div className="space-y-2 mt-3">
                      {Object.entries(q.options).map(([key, value]) => {
                        ...
                        return (
                          <button
                            key={key}
                            onClick={() => !hasAnswered && handleMCQ(section.id, q.id, key)}
                            className={btnClass}
                      ...
                      {selectedMCQ[q.id] && q.explanation && (
                    ...
                    <div className="mt-3">
                      ...
                      {!answers[q.id]?.submitted && (
                        <button
                          ...
                          disabled={!answers[q.id]?.userAnswer}
                          className="w-full mt-2 py-2 bg-purple-600 hover:bg-purple-500 disabled:opacity-40 rounded-xl text-sm font-bold transition-colors"
                        >
                          ✓ تحقق من إجابتي
                        </button>
                      )}
                      ...
                      {answers[q.id]?.submitted && (
                        <button
                          ...
                          className="w-full mt-2 py-2 border border-gray-700 rounded-xl text-xs text-gray-400 hover:border-gray-500 transition-colors"
                        >
                          {showModel[q.id] ? 'إخفاء النموذج' : '📋 عرض الإجابة النموذجية'}
                        </button>
                      )}

                      {showModel[q.id] && q.modelAnswer && (
                        <div className="mt-2 p-3 bg-gray-800 rounded-xl border border-green-800">
                          ...
                          {q.arabicModelAnswer && (
                            <p className="text-gray-400 text-xs mt-2 leading-relaxed">{q.arabicModelAnswer}</p>
                          )}
                          {q.explanation && (
                            <p className="text-purple-300 text-xs mt-2 leading-relaxed border-t border-gray-700 pt-2">💡 {q.explanation}</p>
                          )}
                        </div>
                      )}

                      {q.type === 'composition' && q.writingTips && (
                        <div className="mt-3 p-3 bg-gray-800 rounded-xl border border-yellow-800">
                          ...
                          <ul className="space-y-1">
                            {q.writingTips.map((tip, i) => (
                              <li key={i} className="text-gray-300 text-xs">← {tip}</li>
                            ))}
                          </ul>
                        </div>
                      )}
