    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          lesson: 'Bac English Exam Practice Unit 3'
        }),
      });
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-4" dir="rtl">
      <div className="max-w-2xl mx-auto">
        ...
        <Link href="/lessons/unit3" className="text-purple-400 text-sm mb-6 block">
          → العودة للوحدة الثالثة
        </Link>

        <p className="text-purple-400 text-xs font-bold tracking-widest uppercase mb-1">
          Unit 3 — تدرّب على الامتحان
        </p>
        ...
        {unit3Exam.sections.map((section) => (
                  ...
                  {q.type !== 'mcq' && (
                    <div className="mt-3">
                      ...
                      {!answers[q.id]?.submitted && (
                        <button
                          onClick={() => submitAnswer(q.id, q.question, answers[q.id]?.userAnswer || '', 'modelAnswer' in q ? q.modelAnswer : '')}
