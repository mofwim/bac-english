    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          lesson: 'Bac English Exam Practice Unit 4'
        }),
      });
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-4" dir="rtl">
      <div className="max-w-2xl mx-auto">
        ...
        <Link href="/lessons/unit4" className="text-purple-400 text-sm mb-6 block">
          → العودة للوحدة الرابعة
        </Link>

        <p className="text-purple-400 text-xs font-bold tracking-widest uppercase mb-1">
          Unit 4 — تدرّب على الامتحان
        </p>
        ...
        {unit4Exam.sections.map((section) => (
            ...
            <div className="space-y-4">
              {section.questions.map((q, idx) => (
                <div key={q.id} className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
                  ...
                  {!translated[q.id] && (
                    <button onClick={() => setTranslated(prev => ({ ...prev, [q.id]: true }))} className="text-xs text-gray-500 border border-gray-700 rounded-lg px-3 py-1 mb-3 hover:border-gray-500 transition-colors">
                      🌐 ترجمة
                    </button>
                  )}
                  ...
                  {q.type !== 'mcq' && (
                    <div className="mt-3">
                      ...
                      {!answers[q.id]?.submitted && (
                        <button onClick={() => submitAnswer(q.id, q.question, answers[q.id]?.userAnswer || '', 'modelAnswer' in q ? q.modelAnswer : '')} disabled={!answers[q.id]?.userAnswer} className="w-full mt-2 py-2 bg-purple-600 hover:bg-purple-500 disabled:opacity-40 rounded-xl text-sm font-bold transition-colors">
                          ✓ تحقق من إجابتي
                        </button>
                      )}
