    ...
    return (
      <main className="min-h-screen bg-gray-950 text-white p-6" dir="rtl">
        <div className="max-w-2xl mx-auto">
          ...
          <Link href="/lessons/unit5" className="text-purple-400 text-sm mt-4 block">→ العودة</Link>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-6" dir="rtl">
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-purple-400 text-xs font-bold tracking-widest uppercase mb-1">Unit 5</p>
          ...
          {section.questions.map((q) => (
            <div key={q.id} className={`bg-gray-900 rounded-2xl p-5 border transition-colors duration-300 ${revealed[q.id] ? 'border-purple-500' : 'border-gray-800'}`}>
              ...
              <div className="flex gap-3 mt-4">
                {!translated[q.id] && (
                  <button onClick={() => setTranslated(prev => ({ ...prev, [q.id]: true }))} className="flex-1 py-2 rounded-xl text-sm font-bold border border-gray-700 text-gray-300 hover:border-gray-500 transition-colors">
                    🌐 ترجمة
                  </button>
                )}
                {!revealed[q.id] && (
                  <button onClick={() => setRevealed(prev => ({ ...prev, [q.id]: true }))} className="flex-1 py-2 rounded-xl text-sm font-bold bg-purple-600 hover:bg-purple-500 text-white transition-colors">
                    ✓ الجواب
                  </button>
                )}
              </div>
        ...
        {completedCount === totalCount && totalCount > 0 && (
          <div className="mt-8 bg-gray-900 rounded-2xl p-6 border border-green-500 text-center">
            ...
            <Link href="/lessons/unit5" className="inline-block bg-purple-600 hover:bg-purple-500 text-white text-sm font-bold py-2 px-6 rounded-xl transition-colors">
              → القسم التالي
            </Link>
          </div>
        )}
