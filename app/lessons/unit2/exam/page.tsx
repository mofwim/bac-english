...
export default function Unit2ExamPage() {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          lesson: 'Bac English Exam Practice Unit 2'
        }),
      });
  ...
  const checkMCQ = (questionId: string, selected: string, correct: string) => {
            ...
            <div className="space-y-4">
              {section.questions.map((q, idx) => (
                <div key={q.id} className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
                  ...
                  {q.type === 'mcq' && 'options' in q && (
                    <div className="space-y-2 mt-3">
                      {Object.entries(q.options).map(([key, value]) => {
                        ...
                        if (!hasAnswered) {
                          btnClass += "border-gray-700 bg-gray-800 hover:border-purple-500";
                        } else if (isCorrect) {
                        ...
                        return (
                          <button
                            key={key}
                            onClick={() => !hasAnswered && checkMCQ(q.id, key, q.correctAnswer)}
                            className={btnClass}
                  ...
                  {(q.type === 'answer' || q.type === 'rewrite' || q.type === 'rewriteCorrect' || q.type === 'writeQuestion' || q.type === 'functional' || q.type === 'correctVerb' || q.type === 'clause' || q.type === 'translateToArabic' || q.type === 'translateToEnglish' || q.type === 'findWord' || q.type === 'composition') && (
                    <div className="mt-3">
                      <textarea
                      ...
                      {!answers[q.id]?.submitted && (
                        <button
                          onClick={() => submitAnswer(
                            ...
                            'modelAnswer' in q ? q.modelAnswer : ''
                          )}
                      ...
                      {showModel[q.id] && 'modelAnswer' in q && (
                        <div className="mt-2 p-3 bg-gray-800 rounded-xl border border-green-800">
                          ...
                          <p className="text-white text-sm leading-relaxed" dir="ltr">
                            {q.modelAnswer}
                          </p>
                          {'arabicModelAnswer' in q && q.arabicModelAnswer && (
                            <p className="text-gray-400 text-xs mt-2 leading-relaxed">
                              {q.arabicModelAnswer}
                            </p>
                          )}
                          {'explanation' in q && q.explanation && (
                            <p className="text-purple-300 text-xs mt-2 leading-relaxed border-t border-gray-700 pt-2">
                              💡 {q.explanation}
                            </p>
                          )}
                        </div>
