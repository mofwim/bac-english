...
export default function SectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  if (!section) return (
    <main className="min-h-screen bg-gray-950 text-white p-6" dir="rtl">
      <div className="max-w-2xl mx-auto">
        ...
        <Link href="/lessons/unit12" className="text-purple-400 text-sm mt-4 block">→ العودة</Link>
      </div>
    </main>
  );
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-6" dir="rtl">
      <div className="max-w-2xl mx-auto">
        <Link href="/lessons/unit12" className="text-purple-400 text-sm mb-6 block">→ العودة للوحدة الثانية عشر</Link>
        <p className="text-purple-400 text-xs font-bold tracking-widest uppercase mb-1">Unit 12</p>
        ...
        {completedCount === totalCount && totalCount > 0 && (
          <div className="mt-8 bg-gray-900 rounded-2xl p-6 border border-green-500 text-center">
            ...
            <Link href="/lessons/unit12" className="inline-block bg-purple-600 hover:bg-purple-500 text-white text-sm font-bold py-2 px-6 rounded-xl transition-colors">→ القسم التالي</Link>
          </div>
        )}
      </div>
    </main>
